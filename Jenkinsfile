node{
    ...
    stage('git code checkout'){
        try{
            ...
            git 'https://github.com/chai-098/star-agile-insurance-project.git'
        }
        catch(Exception e){
            ...
            ${BUILD_URL}''', subject: 'Job ${JOB_NAME} ${BUILD_NUMBER} is failed', to: 'kumarchaitu.9669@gmail.com'
        }
    }
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t chaitanyakumarm123/insure-me:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u chaitanyakumarn -p ${dockerHubPassword}"
        sh "${dockerCMD} push chaitanyakumarn123/insure-me:${tagName}"
